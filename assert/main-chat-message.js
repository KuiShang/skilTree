...
class Message {
    ...
    testUrlAndFace(msg) {
        ...
        let msgBody_ = _msgBody_.replace(/((ht|f)tps?):\/\/[\w\-]+(\.[\w\-]+)+([\w\-\.,@?^=%()&:\/~\+#]*[\w\-\@?^=%&\/~\+)#])?/g, function (s, $1, index) {
            // if (/^(\d+)(\.\d+){1,2}$/.test(s)) {
            //     return s;
            // }
...
class FileMsg extends Message {
    ...
    getTemplate(msg) {
        ...
        let state = validityDate(sendtime, msg.FileStatus);
...
class RevokeMsg extends Message {
    ...
    getTemplate(msg) {
        return '<div class="list-revoke clearfix">'
            + '<div class="text-wrapper">' + msg.msgbody + '</div>'
            + '</div>';
    }
